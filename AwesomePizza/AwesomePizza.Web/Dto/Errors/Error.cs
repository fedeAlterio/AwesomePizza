...
public record Error(string Message, string ErrorCode)
{
    ...
    public static Error OrderInWrongState(OrderId orderId, OrderEventType type) => new($"Order {orderId.Value} in wrong state {type}", ErrorCodes.ORDER_WRONG_STATE);
    public static Error DishNotFound(DishId dishId) => new($"Dish not found {dishId.Value}", ErrorCodes.DISH_NOT_FOUND);
    public static Error DuplicatedDish(DishName dishName) => new($"A dish with the same name already exists {dishName.Value}", ErrorCodes.DUPLIACTED_DISH);
